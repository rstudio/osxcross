...
if [ "$PLATFORM" != "Darwin" ]; then
  EXTRACONFFLAGS+="--with-ld=$TARGET_DIR/bin/aarch64-apple-$TARGET-ld "
  EXTRACONFFLAGS+="--with-as=$TARGET_DIR/bin/aarch64-apple-$TARGET-as "
fi
...
pushd $TARGET_DIR/aarch64-apple-$TARGET/include &>/dev/null
